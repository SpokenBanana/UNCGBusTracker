...
the tablet in each bus allows the driver to update the route they're
going, but UNCG doesn't enforce their drivers to update that information.
So, when my app colors a bus green because the bus is a
"Spartan Village" bus, it might actually be going on the "Campus
Loop" route. It still even says they're going a different route on the
...
bus near a stop, but it's not actually going to that stop.
I've reached out to UNCG to send reminders to their drivers to update
which route they are going on, but nothing changed :)

Because of this, I feel like people would blame my app for misreporting
the status of the buses because they wouldn't know about UNCG not
...
Because of this, I decided to at least put it up on github for
